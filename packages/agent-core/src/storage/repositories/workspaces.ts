...
export function createWorkspace(input: WorkspaceCreateInput): Workspace {
  ...
  const maxOrder = db.prepare('SELECT MAX(sort_order) as max_order FROM workspaces').get() as
  ...
  db.prepare(
    `INSERT INTO workspaces (id, name, description, color, sort_order, is_default, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, 0, ?, ?)`,
  ).run(id, input.name, input.description || null, input.color || null, order, now, now);
