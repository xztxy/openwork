...
describe('Shell escaping utilities', () => {
  ...
  describe('Windows PowerShell command script quoting', () => {
    function quotePowerShellLiteral(value: string): string {
      return `'${value.replace(/'/g, "''")}'`;
    }

    function buildPowerShellArgsArray(args: string[]): string[] {
      return args.map((arg) => `  ${quotePowerShellLiteral(arg)}`);
    }

    it('should quote paths with spaces as PowerShell single-quoted literals', () => {
      const command = 'C:\\Users\\Li Yao\\AppData\\Local\\Programs\\opencode.exe';
      const quoted = quotePowerShellLiteral(command);
      expect(quoted).toBe(`'${command}'`);
    });

    it('should escape embedded single quotes by doubling them', () => {
      const command = "C:\\Users\\O'Connor\\opencode.exe";
      const quoted = quotePowerShellLiteral(command);
      expect(quoted).toBe("'C:\\Users\\O''Connor\\opencode.exe'");
    });

    it('should preserve unicode in quoted command/args', () => {
      const command = 'C:\\Users\\李耀\\opencode.exe';
      const args = ['--prompt', '修复这个错误'];
      const quotedCommand = quotePowerShellLiteral(command);
      const quotedArgs = buildPowerShellArgsArray(args);

      expect(quotedCommand).toBe(`'${command}'`);
      expect(quotedArgs[1]).toBe("  '修复这个错误'");
    });
  });
